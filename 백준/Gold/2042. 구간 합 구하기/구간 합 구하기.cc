...
void update(vector<i64> &T, i64 start, i64 end, i64 root, i64 idx, i64 diff) {
    ...
    if (start != end) {
        i64 mid = (start + end) / 2;
        update(T, start, mid, 2 * root, idx, diff);
        update(T, mid + 1, end, 2 * root + 1, idx, diff);
    }
}
