...
ll det(ll n, vector<elem> M) {
	vector<ll> rnd;
	mt19937 rng(0x14004);
	auto randint = [&rng](ll lb, ll ub) {
		return uniform_int_distribution<ll>(lb, ub)(rng);
	};
	for (ll i = 0; i < n; i++) rnd.push_back(randint(1, mod - 1));
	for (auto& i : M) {
...
int main() {
	...
	auto v = berlekamp_massey({1, 1, 1, 2, 2, 3, 4, 5, 7, 9, 12, 16, 21, 28, 37, 49, 65, 86, 114, 151, 200, 265});
	for (auto& i : v) {
		cout << i << " ";
	}
	...
	while (T--) {
		ll n;
		cin >> n;
		cout << guess_nth_term({1, 1, 1, 2, 2, 3, 4, 5, 7, 9}, n - 1) << endl;
	}
