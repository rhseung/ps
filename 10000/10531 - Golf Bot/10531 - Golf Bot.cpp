...
int main() {
    ...
    // K_i + K_j = D_k가 되게 하는 (i, j, k) 쌍의 개수
    // (x^K_1 + x^K_2 + ... + x^K_n)^2 는 x^(K_i + K_j) 의 계수가 (i, j) 쌍의 개수
    // 따라서 (x^K_1 + x^K_2 + ... + x^K_n)^2 의 x^D_k 의 계수를 모두 더하면 답

    const int maxK = *ranges::max_element(K);
    const int maxD = *ranges::max_element(D);
    vector poly(max(maxK, maxD) + 1, 0);
    for (const int k : K) {
        poly[k] = 1;
    }

    const auto sq_poly = math::multiply(poly, poly);
    ll ans = 0;
    for (const int d : D) {
        bool ok = false;

        if (d >= 0 && d < sz(poly) && poly[d]) // 한 번에 홀까지
            ok = true;
        else if (d >= 0 && d < sz(sq_poly) && sq_poly[d] > 0) // 두 번에 홀까지
            ok = true;
