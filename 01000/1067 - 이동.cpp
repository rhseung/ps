...
int main() {
    ...
    // 이산 합성곱: (X_1, X_2, X_3) * (Y_1, Y_2, Y_3) = (X_1*Y_1, X_2*Y_2, X_3*Y_3)
    // 따라서, (X_1, X_2, X_3) * (Y_3, Y_2, Y_1) = (X_1*Y_3, X_2*Y_2, X_3*Y_1) 으로 S를 쉽게 구할 수 있음

    // 회전을 고려하는건 그냥 (X_1, X_2, X_3, X_1, X_2, X_3) * (Y_3, Y_2, Y_1, 0, 0, 0) 으로 하면
    // = (  X_1*Y_3,
    //      X_1*Y_2 + X_2*Y_3,
    //      X_1*Y_1 + X_2*Y_2 + X_3*Y_3,
    //      X_2*Y_1 + X_3*Y_2 + X_1*Y_3,
    //      X_3*Y_1 + X_1*Y_2 + X_2*Y_3,
    //      X_3*Y_2 + X_2*Y_1,
    //      X_3*Y_1 ) 가 되고
    // 가운데 부분 3개 중 최대를 찾는게 회전을 고려한 S의 최대. 근데 어차피 양항이라 전체에서 최대 구해도 될듯?
    ...
    cout << *ranges::max_element(C) << endl;
